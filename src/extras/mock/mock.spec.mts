...
describe('mock function', {
	...
	'whenCalledNext is shorthand for whenCalled().times(1)'() {
		const fn = mock().whenCalledNext().thenReturn(7);
	...
	'multiple arguments can be checked'() {
		const fn = mock().whenCalled().with('foo', 'bar').thenReturn('a');
	...
	'argument checks and limits can be combined'() {
		const fn = mock().whenCalled().with('foo').once().thenReturn('a');
	...
	'arguments can be checked with matchers'() {
		const fn = mock().whenCalled().with(isGreaterThan(2)).thenReturn('a');
	...
	'reset clears configuration'() {
		const fn = mock().whenCalled().thenReturn('a');
	...
	'getInvocation throws an assertion error if the mock has not been called enough times'() {
		...
		expect(() => fn.getInvocation(1)).throws(
			'Expected mock to have been called at least 2 time(s), but was called 1 time(s)',
		);
	},
	...
	'getLatestInvocation throws an assertion error if the mock has not been called'() {
		...
		expect(() => fn.getLatestInvocation()).throws(
