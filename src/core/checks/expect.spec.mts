...
describe('expect', {
	'throws TestAssertionError if the given value does not match the given matcher'() {
		assertError(
			() => expect(1, equals(2)),
			TestAssertionError,
			'expect(value, equals(2)) failed:\n1 != 2',
		);
	},
	...
	async 'waits for async matchers'() {
		...
		await expect(wrapper, eventually(hasProperty('value', equals(2))));
		const tm1 = Date.now();
		if (tm1 - tm0 < 50) {
			throw new TestAssertionError('expect did not await matcher');
		}
	},
	...
	async 'errors if a returned promise is not awaited'() {
		...
		const reject = await assertUnhandled(async () => {
			void expect(x, eventually(equals('foo')));
		}, 1000);
